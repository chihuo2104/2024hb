...
export default defineEventHandler(async (event) => {
	...
	const respt3 = await fetch(config.huohuorb + "?module=getnote&challenge=challenge3-comment&t=" + t + "&cid=" + query.cid + "&tk=" + GenerateToken(config.token, t), {method:"POST"})
	...
	if (cynocid !== config.mockvisittoken) {
...
<!--          <h1 class="p-2 font-bold">注意：小红包冒险quiz3还没有写完，请等待二次上传。</h1>-->
          ...
          <script>
            ...
            async function approve () {
              res = await fetch("/api/huohuorb?module=challenge3-approve&cid=${query.cid}&tk=${GenerateMockToken(cynocid,query.cid,t)}&t=${t}")
              resp = await res.text()
            }
            async function commit() {
              saids = document.getElementById("saids").value
              console.log(saids)
              res = await fetch("/api/huohuorb?module=challenge3-comment&comment=" + btoa(saids) + "&cid=${cynocid}")
              resp = await res.text()
              alert("提交成功！")
              location.reload()
            }
